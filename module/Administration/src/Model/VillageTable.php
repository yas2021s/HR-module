...
class VillageTable extends AbstractTableGateway
{
	protected $table = 'adm_village'; //tablename
	...
	public function getReport($data)
	{
		...
		$select->from(array('v'=>$this->table))
				->join(array('b'=>'adm_block'),'v.block = b.id', array('district'));
		if($data['district'] != '-1'){
			$select->where(array('b.district'=>$data['district']));
		}
		if($data['block'] != '-1'){
			$select->where(array('v.block'=>$data['block']));
		}
	    $selectString = $sql->getSqlStringForSqlObject($select);
		$results = $adapter->query($selectString, $adapter::QUERY_MODE_EXECUTE)->toArray();
		return $results;
