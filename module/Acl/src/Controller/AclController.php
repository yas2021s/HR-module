	...
	*/
    ...
    {  
		...
		if ($this->getRequest()->isPost()):
			...
			$thirdmax_id = $this->getDefinedTable(Acl\RolesTable::class)->getMaxLimitOffset('id');
			$next_id = $thirdmax_id + 1;
			$max_id = $this->getDefinedTable(Acl\RolesTable::class)->getMax('id');
			if($next_id<$max_id && !in_array($next_id,array(100,99))):
				$data = array(
						'id'          => $next_id, 
						'role'	      => $form['role'],
    ...
    {  
		...
		if ($this->getRequest()->isPost()):
			...
			if($form['process_id']!='-1' && $form['role_id']!='-1'):
				...
				$activity = ($processdtl['activity']=='1')?$form['activity']:'0';
