	...
	* */
	public function submitpayrollAction()
	{
		...
		foreach($this->getDefinedTable(Hr\TempPayrollTable::class)->getAll() as $temp_payroll):
			...
			// echo '<pre>';print_r($payroll_data); exit;
			if($result > 0):
				foreach($this->getDefinedTable(Hr\PaystructureTable::Class)->get(array('employee'=> $temp_payroll['employee'])) as $pay_detail):
					...
					// echo '<pre>';print_r($paydetail_data); exit;
		   ...
		   $yearMonth=$form['year'].'-'.$form['month'];
			...
			$voucher_date=$yearMonth.'-'.$day;
				...
				if($result1):
					...
					//API 
					try {
						$config = $this->getServiceLocator()->get('config')['finance_api'];
						$client = new \Laminas\Http\Client($config['url']);
						$client->setMethod(\Laminas\Http\Request::METHOD_POST);
						$client->setHeaders([
							'Content-Type' => 'application/json',
							'Authorization' => 'Bearer ' . $config['token'], // optional
						]);
						...
						$payload = [
							...
							'net_pay' => str_replace(",", "", $payrollNetPay),
							'submitted_by' => $this->_author,
						];
						$client->setRawBody(json_encode($payload));
						$response = $client->send();
					
						if (!$response->isSuccess()) {
							error_log("Finance API error: {$response->getStatusCode()} - {$response->getBody()}");
						}
					
					} catch (\Exception $e) {
						error_log("Finance API exception: " . $e->getMessage());
					}
