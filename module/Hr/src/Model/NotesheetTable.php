...
class NotesheetTable extends AbstractTableGateway 
{
	protected $table = 'hr_notesheet'; //tablename
	...
	*/
	...
	{
		$where = ( is_array($param) )? $param: array('v.id' => $param);
		$adapter = $this->adapter;
	 ...
	 * @return Array
	 */	
	public function getDateWise($column, $year, $month,$where=NULL)
	{	
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
		$con = array(
					'id','date','number','priority_type','subject','description','status','author','created','modified',
	...
	public function getLastNo($no)
	{
		...
		$select->from($this->table);
		$select->columns(array(
				'max' => new Expression('MAX(number)')
		));
		
		$select->where->like('number',$no."%");
    ...
    // Main query
    $select = $sql->select();
    $select->from(['ns' => $this->table])
           ->join(['a' => 'sys_activity_logs'], 'ns.id = a.process_id')
           ->join(['latest' => $subSelect], 'a.process_id = latest.process_id AND a.id = latest.max_id', [])
           ->columns(['*'])
		   ->order('ns.date DESC'); // gets all columns from ns

    if ($where !== null) {
        $select->where($where);
    }
	$select->where->notEqualTo('ns.author', $user);
	...
	public function getActionByMe($where = NULL,$user)
	{
		...
		$select->from(['ns' => $this->table])
			->join(['a' => 'sys_activity_logs'], 'ns.id = a.process_id')
			->order('ns.date DESC')
			->where->notEqualTo('ns.author', $user);
