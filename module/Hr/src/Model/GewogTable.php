...
class GewogTable extends AbstractTableGateway 
{
	protected $table = 'hr_gewog'; //tablename
	...
	{  
	    ...
	    $sql = new Sql($adapter);
	    $select = $sql->select();
	    $select->from(array('g' => $this->table))
	...
	{
		$where = ( is_array($param) )? $param: array('g.id' => $param);
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->from(array('g' => $this->table))
