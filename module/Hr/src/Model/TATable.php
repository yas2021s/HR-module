...
class TATable extends AbstractTableGateway 
{
	protected $table = 'hr_ta'; //tablename
	...
	*/
	...
	{	
		...
		$select = $sql->select();
		$con = array(
					'id','date','ta_no','estimated_expense','advance','employee','status','author','created','modified',
	...
	public function getLastNo()
	{
		...
		$select->from($this->table);
		$select->columns(array(
				'max' => new Expression('MAX(ta_no)')
		));
		
		//$select->where->like('ta_no',$no."%");
    ...
    $select->from(array('ta' => $this->table))
           ->join(array('c' => 'hr_ta_claim'), 'c.ta= ta.id', array(), 'left')
           ->where($param)
           ->where->isNull('c.ta');
