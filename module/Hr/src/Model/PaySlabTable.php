...
class PaySlabTable extends AbstractTableGateway 
{
	...
	*/
	...
	/**Find the Slab */
	public function getPaySlabForTotalEarning($totalEarning)
	{  
		$adapter = $this->adapter;
		$sql = new Sql($adapter);
		$select = $sql->select();
		$select->from($this->table);
		$select->where->expression($totalEarning . ' BETWEEN from_range AND to_range');
		$select->where(array('pay_head'=>44));
	 	$selectString = $sql->getSqlStringForSqlObject($select);
