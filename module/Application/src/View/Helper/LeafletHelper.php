...
class LeafletHelper extends AbstractHelper
{
	...
	public function __invoke($map_id, $districts, $locations, $basePath)
	{
		$districtList = '';
		foreach($districts as $District):
			$districtList .= "{'type':'Feature','properties': {dName: '".ucfirst(strtolower($District['district']))."', status: '".$District['status']."'}, 'geometry': { 'type': 'Polygon', 'coordinates':".$District['coordinates']."},},";
		endforeach;
		...
		$marker = 1; foreach($locations as $Location): if(!empty($Location['coordinates']) && strpos($Location['coordinates'], ',')):
			...
			endswitch;
			$locationList .= "var marker".$marker." = L.marker([".$Location['coordinates']."], {icon: ".$Icon."}).bindPopup('<table><tr><th>Location : </th><td>".$Location['location']."</td></tr><tr><th>Code: </th><td>".$Location['location_code']."</td></tr></table>');";
			$markerList .= 'marker'.$marker++.",";
		endif; endforeach;
		
		$overlayMaps = "var overlayMaps = {'Location': Locations,};";
		$options = "var options = {center: [27.477173, 90.422339],zoomSnap: 0.2,zoom: 7.6,layers: [googleSatellite, Locations],fullscreenControl: true,};";
