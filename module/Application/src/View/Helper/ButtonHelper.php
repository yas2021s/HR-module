			...
			case 1: /** Table Action Button */
				...
				foreach($this->aclTable->renderButton(array('parent_page' => $acl_id, 'button_position' => 1), $user_role,$highestRole) as $row):
					if($status == $row['permission_level']):
						$button = ($button!="")? $button:"<div class='btn-group'>";
						$modal ="";
						if($row['btn_type']==2):
						...
						$icon_only = ($row['btn_label']=='0')?'icon-only':'';
						$button.="<a class='btn ".$class." btn-xs' ".$modal." title='".$row['menu']."' href='".$this->view->url($row['route'], array('action' => $row['action'], 'id' => $id))."' >
							<i class='ace-icon ".$row['icon']." ".$icon_only." bigger-110'></i>".$btn_label."</a>";
					endif;
				endforeach;
				if($button!=""):
					$button.="</div>";
			...
			case 2:/** Panel Head Button */
				...
				foreach($this->aclTable->renderButton(array('parent_page' => $acl_id, 'button_position' => 2), $user_role,$highestRole) as $row):
					if($status == $row['permission_level']):
						...
						$btn_label = ($row['btn_label']=='0')?'':$row['btn_label'];
						$icon_only = ($row['btn_label']=='0')?'icon-only':'';
						$button.="<a class='".$class."' ".$modal." title='".$row['menu']."' href='".$this->view->url($row['route'], array('action' => $row['action'], 'id'=>$id))."' >
							<i class='ace-icon ".$row['icon']." ".$icon_only." bigger-110'></i>".$btn_label."</a>";
					endif;
				endforeach;
			break;
			case 3: /** View Page Button */
				...
				foreach($this->aclTable->renderButton(array('parent_page' => $acl_id, 'button_position' => 3), $user_role,$highestRole) as $row):
					if($status == $row['permission_level']):
						...
						$btn_label = ($row['btn_label']=='0')?'':$row['btn_label'];
						$icon_only = ($row['btn_label']=='0')?'icon-only':'';
						$button.="<a ".$modal." style='width: 100%' title='".$row['menu']."' href=".$this->view->url($row['route'], array('action' => $row['action'], 'id' => $id))." class='btn ".$row['class']."'>
							<i class='ace-icon ".$row['icon']." ".$icon_only." bigger-110'></i>".$btn_label."</a>";
					endif;
				endforeach;
			break;
